 ...
 */
 ...
 gst_cuda_kernel_init (GstCudaKernel * filter)
 {
   ...
   filter->num_params = 0;
   memset(&filter->last_mtime, 0, sizeof(GTimeVal));
     ...
     GstCaps * outcaps)
 {
   ...
   filter->has_cuda_memory = features && 
       gst_caps_features_contains (features, "memory:CUDAMemory");
     ...
     GstBuffer * outbuf)
 {
   ...
   /* Launch the kernel */
   dim3 block_size(filter->block_size_x, filter->block_size_y, 1);
 ...
 gst_cuda_kernel_load_module (GstCudaKernel * filter)
 {
   ...
   if (error) {
     ...
     GTimeVal mtime;
     g_file_info_get_modification_time (file_info, &filter->last_mtime);
 ...
 gst_cuda_kernel_reload_if_needed (GstCudaKernel * filter)
 {
   ...
   if (mtime.tv_sec != filter->last_mtime.tv_sec || 
       mtime.tv_usec != filter->last_mtime.tv_usec) {
     GST_INFO_OBJECT (filter, "Kernel file modified, reloading");
     return gst_cuda_kernel_load_module (filter);
   }
