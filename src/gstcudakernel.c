/*
 * GStreamer CUDA Kernel Plugin - FIXED VERSION
 * 
 ...
 */
 ...
 enum
 {
   ...
   PROP_BLOCK_SIZE_X,
   PROP_BLOCK_SIZE_Y,
   PROP_USE_PINNED_MEMORY  /* NEW: Option to use pinned memory */
 };
 ...
 struct _GstCudaKernel
 {
   ...
   gint block_size_x;
   gint block_size_y;
   gboolean use_pinned_memory;  /* NEW: Option to use pinned memory */
   ...
   /* NEW: CUDA device properties */
   size_t available_memory;
   size_t total_memory;
   
   /* NEW: Pinned memory buffers for reuse */
   void *pinned_input;
   void *pinned_output;
 ...
 /* NEW: Helper functions for memory management */
 ...
 #define PLUGIN_VERSION "1.0.1"  /* Bumped version number */
 ...
 gst_cuda_kernel_class_init (GstCudaKernelClass * klass)
 {
   ...
   /* NEW: Add pinned memory property */
     ...
     GstBuffer * outbuf)
 {
   ...
   if (filter->has_cuda_memory) {
     ...
     /* System memory, need to handle differently */
     ...
     if (filter->use_pinned_memory) {
       void *pinned_input = NULL;
       void *pinned_output = NULL;
       ...
       if (!gst_cuda_kernel_ensure_pinned_buffer(filter, buffer_size, 
         ...
         GST_ERROR_OBJECT (filter, "Failed to allocate pinned memory");
         return GST_FLOW_ERROR;
       }
       ...
       if (result != CUDA_SUCCESS) {
         ...
         GST_ERROR_OBJECT (filter, "Failed to allocate GPU memory (%d bytes): %d", 
 ...
 /* NEW: Memory management helper functions */
